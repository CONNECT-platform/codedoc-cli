...
module.exports = {
  ...
  run: async () => {
    ...
    if (shell.test('-d', '.codedoc')) {
      try {
        core_installed = require(path.resolve('.codedoc/package-lock.json')).dependencies['@codedoc/core'].version;
      ...
      shell.echo(
        chalk`{${core_update ? colors.warning : colors.success} #}`
          ...
          + chalk` {bold.${
            core_installed === 'not installed' ? colors.error : core_update ? colors.warning : colors.success
          } ${core_installed}}`
