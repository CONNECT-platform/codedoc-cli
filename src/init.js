...
const checkNotInit = () => {
  if (shell.test('-d', '.codedoc')) {
    ...
    shell.echo(
      chalk`{${colors.error} #} - Run {${colors.highlight} {bold codedoc} serve} to serve docs on localhost.`
    );
    shell.echo(
      chalk`{${colors.error} #} - Run {${colors.highlight} {bold codedoc} build} to build your docs.`
    );
    shell.echo(
      chalk`{${colors.error} #} - Run {${colors.highlight} {bold codedoc} help} for more info.`
    );
    shell.echo();
    process.exit(1);
  }
...
const checkGit = () => {
  if (!shell.which('git')) {
    shell.echo(
      chalk`{${colors.error} # Error:} {underline.bold GIT} is required!`
    );
...
const checkNPM = () => {
  if (!shell.which('npm')) {
    shell.echo(
      chalk`{${colors.error} # Error:} {underline.bold NPM} is required!`
    );
...
const cloneFiles = async () => {
  ...
  try {
    await exec(
      'git',
      'clone https://github.com/CONNECT-platform/codedoc-boilerplate.git'
    );
    ...
    if (
      !shell.test('-e', 'docs/Dockerfile')
      && !shell.test('-e', 'docs/docker-compose.yml')
    ) {
      shell.cp('-f', './codedoc-boilerplate/docs/Dockerfile', './docs/');
      shell.cp(
        '-f',
        './codedoc-boilerplate/docs/docker-compose.yml',
        './docs/'
      );
    }
