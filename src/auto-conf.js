...
export const config = /*#__PURE__*/configuration({
  ...
  ${
    github
      ? `dest: {
    namespace: '/${github.repo}'${spaces(
          SPACES_COUNT - 16 - github.repo.length
        )}// --> your github pages namespace. remove if you are using a custom domain.
  },`
      : ''
  }
  page: {
...
`;
  ...
  fs.writeFile(path.join('.codedoc', 'config.ts'), conf, err => {
    if (err) {
      ...
      shell.echo(
        chalk`{${colors.success} #} Modify configs by editing {underline.italic {${colors.link} .codedoc/config.ts}}.`
      );
    }
